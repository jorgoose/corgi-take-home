...
/* -------------------------------------------------------------------------- */
/*  ChartStyle – injects CSS custom properties for chart colours               */
/* -------------------------------------------------------------------------- */
        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const indicatorColor =
              color ??
              (item as Record<string, unknown>).fill as string ??
              (item as Record<string, unknown>).color as string

            return (
              <div
                key={(item as Record<string, unknown>).dataKey as string ?? index}
                className={cn(
...
>(
  (
    ...
    return (
      ...
      >
        {payload.map((item) => {
          const key = `${nameKey ?? (item as Record<string, unknown>).dataKey ?? (item as Record<string, unknown>).value ?? "value"}`
          const itemConfig = getPayloadConfigFromPayload(
            config,
            item as Record<string, unknown>,
            key
          ...
          return (
            <div
              key={(item as Record<string, unknown>).value as string ?? key}
              className={cn(
